...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/lukaproject/luka/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin luka-$build-x64-$version
    zip -r luka-$build-x64-$version.zip luka-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum luka-*-x64-$version.zip
