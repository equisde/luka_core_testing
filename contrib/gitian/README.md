...
*Setup instructions for a Gitian build of Luka using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Luka CLI
...
```bash
cp luka/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```
./gitian-build.py --setup luka-user x.x.x
```

Where `luka-user` is your Github name and `x.x.x` is the version tag you want to build.
...
```
git clone git@github.com:luka/gitian.sigs.git
git remote add luka-user git@github.com:luka-user/gitian.sigs.git
```
...
To build the most recent tag:
 ...
 `./gitian-build.py --detach-sign --no-commit -b luka-user 2.0.0`
...
```
export NAME=luka-user
export VERSION=2.0.0
gpg --output $VERSION-linux/$NAME/luka-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/luka-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/luka-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/luka-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/luka-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/luka-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[luka/gitian.sigs](https://github.com/luka/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/luka-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/luka-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/luka-osx-*-build.assert
```
