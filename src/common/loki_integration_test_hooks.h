...
#ifndef LUKA_INTEGRATION_TEST_HOOKS_H
#define LUKA_INTEGRATION_TEST_HOOKS_H
...
#endif // LUKA_INTEGRATION_TEST_HOOKS_H

//
// CPP Implementation
//
#ifdef LUKA_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
#include <string.h>
#include <assert.h>
#include <chrono>
#include <thread>
...
, "luka-default-integration-test-mem-name"
, false
};
...
void luka::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void luka::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void luka::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("Luka Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "Luka Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "Luka Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "Luka Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "Luka Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("Luka Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void luka::deinit_integration_test_context()
{
...
std::vector<std::string> luka::separate_stdin_to_space_delim_args(luka::fixed_buffer const *cmd)
{
...
luka::fixed_buffer luka::read_from_stdin_shared_mem()
{
...
void luka::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
...
#endif // LUKA_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
